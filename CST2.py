...
class ClassicalShadowTransformer(torch.nn.Module):
    ...
    def map_obs(self,obs_string):
        mapping = {'X': 1, 'Y': 2, 'Z': 3}
        return [mapping[char] for char in obs_string]

    def map_out(self,out_string):
        return [int(char) for char in out_string]
    
    def shadowmeasure(self,n_sample):
        if self.state_name=='GHZ':
            state=ghz_state(self.n_qubit)[0]
            
        elif self.state_name=='W_state':
            state=w_state(self.n_qubit)[0]

        elif self.state_name=='Zero_state':
            state=zero_state(self.n_qubit)[0]

        else:
            raise ValueError(f"Unrecognized state name: {self.state_name}")
        obs=shadow(self.n_qubit,n_sample)
        out=measure(state,obs)
    
        obs = torch.tensor([self.map_obs(obs_string) for obs_string in obs])
        out = torch.tensor([self.map_out(out_string) for out_string in out])  

        return(obs,out)

    def sample(self, n_sample, sample_size):
        # 初始化存储所有数据的列表
        all_obs = []
        all_out = []
        ...
        # 定义保存路径和文件名
        save_path = './sample'  # 存储到文件所在的根目录
        filename = f'{self.state_name}_{self.n_qubit}_{n_sample}.pkl'
        
        # 创建存储目录（如果不存在）
        os.makedirs(save_path, exist_ok=True)
        
        # 将所有数据保存到文件
        data_to_save = {'obs': all_obs, 'out': all_out}
    ...
    def optimize(self, steps, max_steps=1000, n_sample=1000, lr=0.0001, autosave=10, **kwargs):
        ...
        # 从样本文件中加载所有数据
        sample_file_path = os.path.join('./sample',f'{self.state_name}_{self.n_qubit}_{n_sample}.pkl')
