...
def stabilizer_measure(gs_stb, ps_stb, gs_obs, ps_obs, r):
    ...
    N = Ng//2
    assert 0<=r<=N
    out = numpy.empty(L, dtype=numpy.int64)
    ga = numpy.empty(2*N, dtype=numpy.int64) # workspace for stabilizer accumulation
...
def binary_repr(ints, width = None):
    ...
    '''
    width = numpy.ceil(numpy.log2(numpy.max(ints)+1)).astype(numpy.int64) if width is None else width
    dt0 = ints.dtype
